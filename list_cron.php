...
foreach ($lines as $line) {
    ...
    // Match ANY of the 4 announcement scripts (with or without leading "sudo")
    if (strpos($line, 'playaudio.sh') !== false ||
        ...
        // Extract script name and target file (tolerate optional "sudo")
        if (preg_match('/(?:sudo\s+)?(playaudio\.sh|playglobal\.sh|polite_play\.sh|polite_global\.sh)\s+(.+)$/', $command, $matches)) {
            $script = $matches[1];
